...
cuya clase = la clave YAML (title/duration/note/precio/ancla…), los pasos
solo-mapa con clase hidden-summary, y todo lo CALCULADO (icono por mode,
horarios Sale/Llega, frase de auxilio, SUBIR/BAJAR) marcado data-derived.
"""
import html
import os
import re
...
TRANSIT_STEP = {}
for _day in DAYS:
    for _step in _day.get("steps", []):
        if "transit" in _step:
            TRANSIT_STEP.setdefault(_step["transit"], _step)
...
def row_text(n, refs):
    ...
    if n.get("duration") not in (None, 0, "0"):
        parts.append(f'<span class="duration">{html.escape(str(n["duration"]))}</span>')
    if n.get("note"):
